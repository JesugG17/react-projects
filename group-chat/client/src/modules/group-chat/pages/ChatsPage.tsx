...
export const ChatsPage = () => {
  ...
  return (
    <section className="flex p-2">
      <article className="bg-slate-500 w-full h-[565px] rounded flex flex-col shadow-lg"> 
        <article className="bg-blue-400 w-full flex p-2 text-white">
            Jesus Gastelum
        </article>
        <ul className="flex flex-col gap-5 flex-1 p-2">
          {
            messages.map((message, index) => (
              <li 
                key={index}
                className={`w-auto max-w-[220px] text-clip text-xs rounded p-1 gap-2 flex flex-col text-white ${message.author === "You" ? "self-end bg-green-500" : "self-start bg-blue-400"}`}
              >
