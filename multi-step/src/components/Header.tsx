    ...
    <header className='relative'>
      ...
      <nav className='absolute top-8 flex justify-center w-full md:justify-start md:left-8'>
        <ul className='flex gap-4 md:flex-col md:gap-8'>
          {STEPS.map((step) => (
            <li className='md:flex md:gap-5 md:items-center' key={step.stepNumber}>
              <div
                className={twMerge(
                  'text-white ring-1 ring-white p-4 w-6 h-6 flex justify-center items-center rounded-full',
                  currentStep === step.stepNumber && 'bg-blue-light text-black'
                )}
              >
                {step.stepNumber}
              </div>
