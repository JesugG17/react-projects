...
export const PickAddons = () => {
    ...
    const returnCheckedStyles = (service: string) => {
        if (service === 'Online service' && checked.service) return 'bg-cyan-light border-blue-dark';
        if (service === 'Larger storage' && checked.storage) return 'bg-cyan-light border-blue-dark';
        if (service === 'Customizable profile' && checked.customizable) return 'bg-cyan-light border-blue-dark';
        return '';
    }
        ...
        <ul className="flex flex-col gap-3">
            {
                cards.map( card => (
                    <li
                        ...
                        className={`flex justify-between items-center border px-3 py-2 rounded ${returnCheckedStyles(card.service)}`}
                    >
